class FollowersController < ApplicationController
  ...
  def index
    ...
    @followed = Follower.where("follower_id=?", session[:user_id])
  ...
  def create
    ...
    @follwed = Follower.where("follower_id=?", session[:user_id])
  ...
  def followed
		@followers = Follower.where("follower_id=?", current_user.id).map{|f| f.user}
